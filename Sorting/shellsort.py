...
def shellSort(lst, less_greater,column,tipo_ordenamiento):
    n = lt.size(lst)
    h = 1
    ...
    while (h >= 1):
        for i in range (h,n):
            j = i
            while (j>=h) and less_greater (lt.getElement(lst,j+1),lt.getElement(lst,j-h+1),column,tipo_ordenamiento):
